    ...
    <RevealGroup className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {services.map((service, index) => (
        <Reveal key={service.slug} className={index === 0 ? "sm:col-span-2" : ""}>
          ...
          >
            {index === 0 ? (
              <div className="bg-brand-deep text-white border-brand-line/25 hover:border-brand flex h-full flex-col justify-between rounded-3xl border p-10 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_24px_60px_-12px_rgba(213,175,84,0.4)]">
                <div>
                  ...
                  <p className="text-white/80 mt-3 max-w-md text-sm">{service.summary}</p>
                </div>
